...
//------------------------------------------------------------------------------
int get_ocam_model(ocam_model *myocam_model, const char *filename)
{
...
//------------------------------------------------------------------------------
void cam2world(double point3D[3], const double point2D[2], const ocam_model *myocam_model)
{
...
//------------------------------------------------------------------------------
void world2cam(double point2D[2], const double point3D[3], const ocam_model *myocam_model)
{
...
//------------------------------------------------------------------------------
void create_perspecive_undistortion_LUT_2ch(cv::Mat &mapxy, int width, int height, const ocam_model *ocam_model, float sf)
{
	int i, j;
	
	mapxy = cv::Mat(height, width, CV_32FC2);
	...
	for (i = 0; i<height; i++)
		for (j = 0; j<width; j++)
		{
			...
			M[2] = Nz;
			world2cam(m, M, ocam_model);
			mapxy.at<cv::Vec2f>(i, j)[0] = (float)m[1];
			mapxy.at<cv::Vec2f>(i, j)[1] = (float)m[0];
		}
}
